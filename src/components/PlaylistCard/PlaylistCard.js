...
const PlaylistCard = ({
  ...
  return (
    <div
      className='w-full h-full flex flex-col rounded-xl overflow-hidden bg-[#1A1A1A] shadow-thumbnailCard'
      onMouseEnter={() => setIsHover(true)}
      onMouseLeave={() => setIsHover(false)}
    >
      <div className='relative w-full'>
        <div
          className='absolute flex justify-end w-full h-full '>
          {
            isHover
              ? (
                <Link
      ...
      <div
        ...
        <Link
          to='/'>
          <div
            className='opacity-60 text-sm font-medium mt-1 hover:opacity-100'
          >
            VIEW PLAYLIST
        </div>
        </Link>
